...
Indeed, consider $cal(F)_1 = { B subset U : "diam" B < delta }$. It is a finite cover of $U$, so by VCT, $exists$ countable disjoint subcollection $cal(G)_1 subset cal(F)_1$ s.t. $U subset union.big_(B in cal(G)_1) hat(B)$ . So
...
A simple example of a doubling measure is $cal(L)^n$ on a Euclidean space. For non doubling measure, a theorem similar to UCT is as follows.

#thm([Besikovitch Covering Theorem])[
