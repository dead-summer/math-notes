...
#prf[
WLOG say $nu: cal(F) -> [- infinity, +infinity)$ . Consider
$
...
$
nu(E) = nu(union.big.sq_(j=1)^(infinity) (E sect Q_j)) = sum_(j=1)^(infinity)nu(E sect Q_j) >= 0,
$
...
$
By continuity from above of $nu$ (check) , $F := sect.big_(j=1)^(infinity)E_j$ satisfies
$
...
$
P without P' subset.eq P sect N'
$
which is both positive and negative, hence $P without P'$ is null set. By symmetry, $P triangle P'$ is null.

]

Let $X = P union.sq N$ be Hahn decomposition, set $nu^(+) (E) := nu (E sect P) gt.eq 0$, $nu^(-) (E) := - nu (E sect N) gt.eq 0 , forall E cal(F)$. Then $nu =  nu^(+) - nu^(-)$, which is the Jordan decomposition of the signed measure $nu$.
