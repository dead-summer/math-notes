...
+ #strong[Countable-Cocountable $sigma$-Algebra:] If $X$ is an uncountable set, the collection $ cal(F) := { E subset.eq X mid(|) E "is countable or" E^complement "is countable" } $ is a $sigma$-algebra. To verify this, consider a countable collection of sets ${ E_j }_(j = 1)^oo subset.eq cal(F)$.
  ...
  - If at least one set, say $E_(j_0)$, has a countable complement (i.e., is co-countable), then the complement of the union satisfies $(union.big_(j = 1)^oo E_j)^c = inter.big_(j = 1)^oo E_j^c subset.eq E_(j_0)^c$. Since $E_(j_0)^c$ is countable, any subset of it is also countable. Therefore, $(union.big_(j = 1)^oo E_j)^c$ is countable, which means $union.big_(j = 1)^oo E_j in cal(F)$.
...
#definition[
Let $cal(E) subset.eq cal(P) (X)$ be an arbitrary collection of subsets of $X$. The #strong[$sigma$-algebra generated by $cal(E)$];, denoted $sigma (cal(E))$, is defined as the interinterion of all $sigma$-algebras on $X$ that contain $cal(E)$: $ sigma (cal(E)) := inter.big { cal(F) mid(|) cal(F) "is a" sigma "-algebra on" X "and" cal(E) subset.eq cal(F) }. $
